...
--проставляємо всім без електрроплити
UPDATE @sobes
SET T11_cod_na = s.t11
FROM(
		SELECT acc.AccountId
				, tm.HasHotWater
				, 'електроенергія без стац. ел\пл, місто' AS t11
		FROM @sobes acc
		LEFT JOIN AccountingCommon.UsageObject as uo ON uo.AccountId = acc.AccountId
		LEFT JOIN AccountingCommon.Point as p ON p.UsageObjectId = uo.UsageObjectId
		LEFT JOIN AccountingCommon.TarifficationMethod tm ON tm.PointId = p.PointId
...
--проставляємо в кого є електроплити
UPDATE @sobes
SET T11_cod_na = s.t11
FROM(
		SELECT acc.AccountId
...
--витягаємо борг більше 2 місяців від суми 340 грн.
UPDATE @sobes
SET Borg = s.RestSumm
FROM(
		SELECT a.AccountId
